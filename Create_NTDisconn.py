...
def buildArgsParser():
    ...
    p.add_argument(
        "--filter", default="n", help="Filter Streamlines - enter percentile [y|n]"
    )
...
def main():
    ...
    def define_streamlines(
        streamlines, lesion, reference
    ):  # , NT_weights_SUM):#, weights):
        metric_tractrogram = []
    ...
    if os.path.isfile(out_weights_tractogram_disc):
        ...
        mywarpedimage = ants.apply_transforms(
            fixed=fi, moving=movmap, transformlist=listtransf, interpolator="multiLabel"
        )
        ...
        weights_tractogram = np.asarray(weights_tractogram, dtype=np.float32)
